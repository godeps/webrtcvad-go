...
// fft.go 实现复数FFT和实数FFT
// 基于WebRTC的ComplexFFT实现

// FFT常量
const (
	kMaxFFTOrder = 10    // 最大FFT阶数
	CFFTSFT      = 14    // 复数FFT移位
	CFFTRND      = 1     // 复数FFT舍入
	CFFTRND2     = 16384 // 复数FFT舍入2
	CIFFTSFT     = 14    // 复数IFFT移位
	CIFFTRND     = 1     // 复数IFFT舍入
)

// kSinTable1024 正弦查找表
// 预计算的正弦值，用于FFT旋转因子
var kSinTable1024 = [1024]int16{
...
// ComplexFFT 执行复数FFT
//
// frfi: 输入/输出数组，交替存储实部和虚部 [re0, im0, re1, im1, ...]
// stages: FFT阶数（2^stages = FFT长度）
// mode: 0=低复杂度/低精度，1=高复杂度/高精度
//
// 返回：0=成功，-1=失败
func ComplexFFT(frfi []int16, stages int, mode int) int {
	n := 1 << uint(stages)
	if n > 1024 {
		return -1
	}
		...
		for l < n {
			...
			for m := 0; m < l; m++ {
				...
				for i := m; i < n; i += istep {
					...
					tr32 := (int32(wr)*int32(frfi[2*j]) - int32(wi)*int32(frfi[2*j+1])) >> 15
					ti32 := (int32(wr)*int32(frfi[2*j+1]) + int32(wi)*int32(frfi[2*j])) >> 15

					qr32 := int32(frfi[2*i])
					qi32 := int32(frfi[2*i+1])

					frfi[2*j] = int16((qr32 - tr32) >> 1)
					frfi[2*j+1] = int16((qi32 - ti32) >> 1)
					frfi[2*i] = int16((qr32 + tr32) >> 1)
					frfi[2*i+1] = int16((qi32 + ti32) >> 1)
				}
			}
		...
		for l < n {
			...
			for m := 0; m < l; m++ {
				...
				for i := m; i < n; i += istep {
					...
					tr32 := int32(wr)*int32(frfi[2*j]) - int32(wi)*int32(frfi[2*j+1]) + CFFTRND
					ti32 := int32(wr)*int32(frfi[2*j+1]) + int32(wi)*int32(frfi[2*j]) + CFFTRND

					tr32 >>= (15 - CFFTSFT)
					ti32 >>= (15 - CFFTSFT)

					qr32 := int32(frfi[2*i]) * (1 << CFFTSFT)
					qi32 := int32(frfi[2*i+1]) * (1 << CFFTSFT)

					frfi[2*j] = int16((qr32 - tr32 + CFFTRND2) >> (1 + CFFTSFT))
					frfi[2*j+1] = int16((qi32 - ti32 + CFFTRND2) >> (1 + CFFTSFT))
					frfi[2*i] = int16((qr32 + tr32 + CFFTRND2) >> (1 + CFFTSFT))
					frfi[2*i+1] = int16((qi32 + ti32 + CFFTRND2) >> (1 + CFFTSFT))
				}
			}
...
// ComplexIFFT 执行复数逆FFT
//
// frfi: 输入/输出数组，交替存储实部和虚部
// stages: FFT阶数
// mode: 0=低复杂度/低精度，1=高复杂度/高精度
//
// 返回：缩放因子（>=0）或-1（失败）
func ComplexIFFT(frfi []int16, stages int, mode int) int {
	n := 1 << uint(stages)
	if n > 1024 {
		return -1
	}
		...
		if mode == 0 {
			...
			for m := 0; m < l; m++ {
				...
				for i := m; i < n; i += istep {
					...
					frfi[2*j] = int16((qr32 - tr32) >> uint(shift))
					frfi[2*j+1] = int16((qi32 - ti32) >> uint(shift))
					frfi[2*i] = int16((qr32 + tr32) >> uint(shift))
					frfi[2*i+1] = int16((qi32 + ti32) >> uint(shift))
				}
			}
		} else {
			// mode==1: 高复杂度和高精度模式
			for m := 0; m < l; m++ {
				...
				for i := m; i < n; i += istep {
					...
					tr32 := int32(wr)*int32(frfi[2*j]) - int32(wi)*int32(frfi[2*j+1]) + CIFFTRND
					ti32 := int32(wr)*int32(frfi[2*j+1]) + int32(wi)*int32(frfi[2*j]) + CIFFTRND

					tr32 >>= (15 - CIFFTSFT)
					ti32 >>= (15 - CIFFTSFT)

					qr32 := int32(frfi[2*i]) * (1 << CIFFTSFT)
					qi32 := int32(frfi[2*i+1]) * (1 << CIFFTSFT)

					frfi[2*j] = int16((qr32 - tr32 + round2) >> uint(shift+CIFFTSFT))
					frfi[2*j+1] = int16((qi32 - ti32 + round2) >> uint(shift+CIFFTSFT))
					frfi[2*i] = int16((qr32 + tr32 + round2) >> uint(shift+CIFFTSFT))
					frfi[2*i+1] = int16((qi32 + ti32 + round2) >> uint(shift+CIFFTSFT))
				}
			}
		}
...
// CreateRealFFT 创建实数FFT对象
//
// order: FFT阶数（1 < order <= kMaxFFTOrder）
// 返回：RealFFT对象或nil
func CreateRealFFT(order int) *RealFFT {
	if order <= 1 || order > kMaxFFTOrder {
		return nil
	}
...
// RealForwardFFT 实数前向FFT
//
// realDataIn: 实数输入 (长度 2^order)
// complexDataOut: 复数输出 (长度 2^order + 2)，使用CCS格式
//
// 返回：0=成功，-1=失败
func (fft *RealFFT) RealForwardFFT(realDataIn []int16, complexDataOut []int16) int {
	if realDataIn == nil || complexDataOut == nil {
		return -1
	}

	if len(realDataIn) < fft.n || len(complexDataOut) < fft.n+2 {
		return -1
	}

	// 将实数数据重排为复数格式（交织0虚部）
	for i := 0; i < fft.n/2; i++ {
		fft.work[2*i] = realDataIn[2*i]
		fft.work[2*i+1] = realDataIn[2*i+1]
	}

	// 执行复数FFT
	ComplexFFT(fft.work, fft.order-1, 1)

	// 后处理：将结果转换为实数FFT的CCS格式
	complexDataOut[0] = fft.work[0]
	complexDataOut[1] = 0
	...
	for i := 1; i < fft.n/2; i++ {
		...
		re1 := fft.work[2*i]
		im1 := fft.work[2*i+1]
		re2 := fft.work[2*(fft.n/2-i)]
		im2 := fft.work[2*(fft.n/2-i)+1]

		complexDataOut[2*i] = (re1 + re2) >> 1
		complexDataOut[2*i+1] = (im1 - im2) >> 1
	}

	complexDataOut[fft.n] = fft.work[1]
	complexDataOut[fft.n+1] = 0
...
//
// complexDataIn: 复数输入（CCS格式，长度 2^order + 2）
// realDataOut: 实数输出（长度 2^order）
//
// 返回：缩放因子（>=0）或-1（失败）
func (fft *RealFFT) RealInverseFFT(complexDataIn []int16, realDataOut []int16) int {
	if complexDataIn == nil || realDataOut == nil {
		return -1
	}

	if len(complexDataIn) < fft.n+2 || len(realDataOut) < fft.n {
		return -1
	}

	// 预处理：将CCS格式转换为复数格式
	fft.work[0] = complexDataIn[0]
	fft.work[1] = complexDataIn[fft.n]

	for i := 1; i < fft.n/2; i++ {
		re := complexDataIn[2*i]
		im := complexDataIn[2*i+1]

		fft.work[2*i] = re
		fft.work[2*i+1] = im
		fft.work[2*(fft.n/2-i)] = re
		fft.work[2*(fft.n/2-i)+1] = -im
	}

	// 执行复数IFFT
	scale := ComplexIFFT(fft.work, fft.order-1, 1)

	// 提取实数部分
	for i := 0; i < fft.n; i++ {
		if i%2 == 0 {
			realDataOut[i] = fft.work[i]
		} else {
			realDataOut[i] = fft.work[fft.n-i]
		}
	}
